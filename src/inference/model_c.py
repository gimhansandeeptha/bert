...
class ModelC():
  def __init__(self,bert_model, preprocess_model):
    ...
    self.bert_model = hub.KerasLayer(bert_model, name='BERT_encoder')
    # self.bert_model.layers[0].trainable=False
    bert_keras_model = self.bert_model.signatures['default']
    for layer in bert_keras_model.layers[1:]:
            layer.trainable = False


  def build_classifier_model(self, n_output_classes:int,activation:str, learning_rate:float, n_trainable):
