...
class ModelB():
  def __init__(self,bert_model, preprocess_model):
    ...
    self.bert_model = hub.KerasLayer(bert_model, trainable=False, name='BERT_encoder')
  ...
  def build_classifier_model(self, n_output_classes:int,activation:str, learning_rate:float):
    ...
    # Middle Layers
    net  = tf.keras.layers.Dense(800, activation = 'relu', name='dense_1')(net)
    net = tf.keras.layers.Dropout(0.1)(net)

    net  = tf.keras.layers.Dense(768, activation = 'relu', name='dense_2')(net)
    net = tf.keras.layers.Dropout(0.1)(net)

    net  = tf.keras.layers.Dense(24, activation = 'relu', name='dense_3')(net)
    net = tf.keras.layers.Dropout(0.1)(net)
